...
def _aes_encrypt(text, sec_key):
    print "44: text " + text
    print "44: sec_key " + sec_key
    pad = 16 - len(text) % 16
    print pad
    print chr(pad)
    print (pad * chr(pad))
    text = text + pad * chr(pad)
    print "4444444: text " + text
    print "4444444444444: sec_key " + sec_key
    aes = pyaes.AESModeOfOperationCBC(sec_key, iv='0102030405060708')
    print aes
    ciphertext = ''
    while text != '':
        print "55: aes while "
        print map(lambda xx: (ord(xx)), text[:16])
        aaa = aes.encrypt(text[:16])
        print map(lambda xx: (ord(xx)), aaa)
        ciphertext += aaa
        text = text[16:]
    print "55: aes ciphertext " + ciphertext
    ciphertext = base64.b64encode(ciphertext)
    print "55: aes base64 ciphertext " + ciphertext
    print "55: aes ciphertext ================================================="
    return ciphertext
